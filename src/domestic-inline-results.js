  ...
  function renderSearchDiagnostics(sources) {
    ...
    const blocks = failures.map(source => {
      ...
      const fields = [
        ...
        ["요청 주소", safeUrl(d.targetUrl || source.searchUrl)], ["원래 요청 주소", safeUrl(d.originalTargetUrl)],
        ["실제 주소", safeUrl(d.resolvedUrl)],
        ["검색 입력 단계", d.submissionStage], ["검색 입력 실패", d.submissionFailure],
        ["입력칸 최대 글자 수", d.inputMaxLength], ["입력 검색어 글자 수", d.submittedQueryLength],
