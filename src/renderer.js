...
function renderVerifiedSpuRows(file, products) {
  ...
  $("#excel-preview-rows").innerHTML = products.length ? products.map((p, i) => {
    ...
    return '<tr class="excel-product-row excel-verified-spu-row"><td><input type="checkbox" data-excel-product-select="' + encodeURIComponent(keys[i]) + '"></td><td>' + image + '</td><td><b>' + text(p.articleNumber) + '</b><small> SPU ' + text(p.spuId) + '</small></td><td>' + text(p.title) + '<details><summary>원본 사이즈 ' + p.optionCount + '행</summary>' + options + '</details></td><td>' + text(p.brandName) + '</td><td>' + (p.hasPriceData ? money(p.averagePrice) : '미확인') + '</td><td>' + text(p.hasSalesData ? p.sales30dRaw : '미확인') + '</td><td>' + text(p.hasLocalSalesData ? p.localSales30dRaw : '미확인') + '</td><td>' + text(p.verificationStatus) + '</td>' + renderRawExcelDomesticCell(keys[i], p, excelPreviewSearchResults.get(keys[i])) + '</tr>';
