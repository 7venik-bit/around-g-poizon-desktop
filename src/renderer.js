...
let selectedCategorySelections = new Map();
...
function renderCombinedBrandPreviewPage(offset = 0) {
  ...
  $("#excel-preview").classList.add("product-view");
  $("#excel-preview-loading").hidden = true;
  $("#excel-preview-grid").hidden = false;
...
window.aroundG.onDomesticSearchProgress?.((payload = {}) => {
  ...
  if (payload.checkpoint?.products || payload.checkpoint?.sources) {
    ...
    activeDomesticCheckpoint = payload.checkpoint;
  }
  const overlay = $("#domestic-search-overlay");
...
function updateExcelPreviewSelectionUi(pageKeys = []) {
  ...
  if (selectAll) {
    ...
    const combined = Array.isArray(activeExcelPreview?.combinedProducts);
...
async function showDomesticRecoveryNotice(filePath) {
  ...
  button.addEventListener('click', () => {
    ...
    domesticRecoveryResumeRequested = true;
    selectedExcelPreviewProducts.clear();
...
function excelPreviewProductSourcePath(product = {}, file = {}) {
...
function renderCategoryButtons() {
  $("#category-buttons").innerHTML = explorerMeta.categories.map((category) =>
    `<button class="category-button ${selectedCategorySelections.has(category) ? "selected" : ""}" data-category="${text(category)}" aria-pressed="${selectedCategorySelections.has(category)}"><strong>${text(category)}</strong><span>›</span></button>`
  ).join("");
  const details = CATEGORY_DETAILS[selectedCategory] || ["전체 상품"];
  const selectedDetails = selectedCategorySelections.get(selectedCategory) || new Set();
  $("#category-detail-section").hidden = false;
  $("#category-detail-buttons").innerHTML = details.map((detail) =>
    `<button type="button" class="category-detail-button ${selectedDetails.has(detail) ? "selected" : ""}" data-category-detail="${text(detail)}" aria-pressed="${selectedDetails.has(detail)}">${text(detail)}</button>`
  ).join("");
...
function renderCategoryFavoriteBrands({ reset = false } = {}) {
  ...
  $("#category-search").disabled = !selectedCategoryPairs().length || !categoryBrandIds.size;
}
...
document.addEventListener("click", async (event) => {
  ...
  if (retryId) {
    ...
    if (row) {
      selectedLedgerIndex = -1; fillLedgerForm(row); renderCapturedLedgerRows(); document.querySelector('[data-view="ledger"]')?.click();
      if (!row.orderEvidence) { $("#ledger-status").className="status error"; $("#ledger-status").textContent="이전 기록에는 주문상세 확인 정보가 없습니다. 해당 주문상세를 다시 인식해 주세요."; }
    }
  }
  ...
  if (detail) {
    toggleCategoryDetail(selectedCategory, detail);
    localStorage.setItem("around-g-last-category", JSON.stringify({ selections: selectedCategoryPairs() }));
    renderCategoryButtons();
  }
...
$("#completed-brand-domestic-search")?.addEventListener("click", async () => {
  ...
  const selectedDownloads = selectedBrandsForExport()
    ...
    .filter(Boolean)
    .filter((file, index, files) => files.findIndex((item) => brandImportPathKey(item.path) === brandImportPathKey(file.path)) === index);
  const fallbackDownloads = completedDownloadBrands()
    .map((brand) => latestCompletedBrandDownload(brand))
    .filter(Boolean);
  const files = selectedDownloads.length ? selectedDownloads : fallbackDownloads;
  const status = $("#brand-status");
  if (!files.length) {
    if (status) {
      status.className = "status error";
...
$("#excel-preview-search-selected")?.addEventListener("click", async () => {
  ...
  try {
    ...
    // Recovery is activated only by the explicit "미완료 검색 이어하기"
    // action; otherwise splitting the request by retailer can delay or hide
    // every source behind the first incomplete retailer.
    globalThis.aroundGActiveDomesticRecovery = null;
    const resumeRecovery = domesticRecoveryResumeRequested;
    domesticRecoveryResumeRequested = false;
    const recovery = resumeRecovery
      ? await prepareDomesticRecovery(keys.map(key => excelPreviewProductCache.get(key)), activeExcelPreview?.file?.path || 'product-list')
      : null;
    if (runId !== excelPreviewSearchRunId) return;
...
$("#excel-preview-prev")?.addEventListener("click", () => {
  ...
  else void showExcelPreview(activeExcelPreview.file, Math.max(0, activeExcelPreview.offset - activeExcelPreview.limit), activeExcelPreview.filters);
});
...
function selectedCategoryPairs() {
  const pairs = [];
  for (const [category, details] of selectedCategorySelections || []) {
    for (const detail of details || []) pairs.push({ category, detail });
  }
  if (!pairs.length && selectedCategoryDetail) pairs.push({ category: selectedCategory, detail: selectedCategoryDetail });
  return pairs;
}
...
function toggleCategoryDetail(category, detail) {
  ...
  else if (detail === "전체 상품") {
    details.clear();
    details.add(detail);
  } else {
    details.delete("전체 상품");
    details.add(detail);
  }
  if (details.size) selectedCategorySelections.set(category, details);
  else selectedCategorySelections.delete(category);
  selectedCategoryDetail = "";
  const remaining = selectedCategoryPairs();
  if (remaining.length) selectedCategoryDetail = remaining.at(-1).detail;
}
...
$("#ledger-open-musinsa")?.addEventListener("click", async () => {
  ...
  const status=$("#ledger-status");status.className="status";status.textContent="무신사 자동 로그인 → 마이 → 주문 내역으로 이동하고 있습니다.";setLedgerBusy(true);
  try {
    const result=await window.aroundG.openMusinsaLedger();
    if(!result?.ok || result.automaticLogin?.ok===false){status.className="status error";status.textContent=ledgerFlowMessage(result);return;}
    status.className="status success";status.textContent=result.stage==="detail"
      ? "선택한 주문상세가 열려 있습니다. 주문상세 인식을 눌러 상품 정보를 가져오세요."
      : result.message || "무신사 주문 내역에서 기록할 주문의 주문상세를 클릭한 뒤 ② 주문상세 인식을 눌러주세요.";
  } catch {status.className="status error";status.textContent="무신사 주문 내역을 열지 못했습니다. 연결 상태를 확인해 주세요.";}
...
$("#ledger-captured-list")?.addEventListener("click", event => {
  if(ledgerBusy)return;
  const button=event.target.closest("[data-ledger-captured]");if(!button)return;
  if(selectedLedgerIndex>=0 && selectedLedgerProof?.captureId) Object.assign(capturedLedgerRows[selectedLedgerIndex],ledgerFormRow());
  selectedLedgerIndex=Number(button.dataset.ledgerCaptured);fillLedgerForm(capturedLedgerRows[selectedLedgerIndex]);renderCapturedLedgerRows();
});
