...
const PROFILES = [
  ...
  {id:'salesforce',hosts:['puma.com','underarmour.co.kr','asics.com','vans.co.kr','crocs.co.kr'],options:['.select-size option','[data-attr="size"] button','[data-attr="size"] input','[class*="size" i] button']},
  ...
  const visit=async(prefix,depth)=>{
    ...
    for(const option of choices){
      ...
      if(leaf||option.inStock===false){
        ...
        try { await select(group,option);await settle();await visit(path,depth+1); }
...
export function captureNativeStockControls() {
  ...
  const excluded='header,footer,nav,[class*="recommend" i],[class*="related" i],[class*="review" i],[class*="delivery" i],[class*="shipping" i],[class*="sizeguide" i],[class*="sizetable" i],[class*="size-guide" i],[class*="size-chart" i]';
  const scope=document.querySelector('.cdtl_col_rgt')||document;
  ...
  const groups=selects.flatMap(el=>{
    ...
    if(!/size|color|colour|option|사이즈|색상|컬러|옵션|선택/i.test(label+' '+(el.innerText||el.textContent)))return [];
  ...
  if(radios.length)return {groups:radios};
