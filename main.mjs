...
async function september10AddMatchConfidence(data, input) {
  const source = {
...
async function september10VerifyApprovedNaverDomesticProducts(products = [], {
  articleNumber = "",
  brand = "",
  title = "",
...
async function september10OpenRenderedSizeOptions(searchWindow) {
...
function september10RenderedStockSelectors(store = "") {
...
async function september10ClickRenderedProductCard(searchWindow, productUrl, searchResultsUrl = "") {
  if (!searchWindow || searchWindow.isDestroyed()) return false;
...
function september10BrowserWindowUsable(window) {
...
async function september10LoadOfficialPageForAutomation(searchWindow, targetUrl, timeoutMs = 8_000) {
  if (!september10BrowserWindowUsable(searchWindow)) return false;
  ...
  const ready = new Promise((resolve) => {
    ...
    const timer = setTimeout(() => finish(september10BrowserWindowUsable(searchWindow)), timeoutMs);
...
function september10ClosedInternalSearchResult(stage = "unknown") {
  return {
    ok: false,
    submitted: false,
    canceled: true,
...
async function september10OpenOfficialMallInternalSearch(homepageUrl, query) {
  ...
  searchWindow.webContents.setWindowOpenHandler(({ url }) => {
    if (/^https?:\/\//i.test(url) && september10BrowserWindowUsable(searchWindow)) {
  ...
  try {
    const homepageReady = await september10LoadOfficialPageForAutomation(searchWindow, homepage.href);
    if (!september10BrowserWindowUsable(searchWindow)) return september10ClosedInternalSearchResult(stage);
    ...
    if (login.required) {
      ...
      await september10LoadOfficialPageForAutomation(searchWindow, homepage.href);
      if (!september10BrowserWindowUsable(searchWindow)) return september10ClosedInternalSearchResult(stage);
    }
    ...
    if (!september10BrowserWindowUsable(searchWindow)
      || /Object has been destroyed|Render frame was disposed|WebContents was destroyed/i.test(message)) {
      return september10ClosedInternalSearchResult(stage);
    }
    throw error;
  }
}

async function september10LoadNaverFashionTownResultPage(searchWindow, targetUrl, query) {
...
async function september10RenderedSearchSourceResult(source, articleNumber, brand = "", title = "", securityRetry = 0, searchAttempt = null, sharedNaverSession = null, generation = domesticSearchGeneration) {
  ...
  try {
    ...
    if (reuseNaverSearch) {
      ...
      */
      ...
      if (directNaverFashionResult) {
        const resultPage = await september10LoadNaverFashionTownResultPage(
          ...
          if (!shoppingHomeOpened) {
            ...
            if (securityRequired && securityRetry < 1) {
              ...
              if (verified) {
                ...
                searchWindow = null;
                return september10RenderedSearchSourceResult(source, articleNumber, brand, title, securityRetry + 1, searchAttempt, sharedNaverSession, generation);
              }
            }
          ...
          if (!fashionTownOpened) {
            ...
            if (securityRequired && securityRetry < 1) {
              ...
              if (verified) {
                searchWindow.destroy();
                searchWindow = null;
                return september10RenderedSearchSourceResult(source, articleNumber, brand, title, securityRetry + 1, searchAttempt, sharedNaverSession);
              }
            }
            return renderedSearchFailure(
    ...
    let content = await searchWindow.webContents.executeJavaScript(`(() => {
      ...
      const structuralCardLinks = allLinks.filter((link) => {
        ...
        for (let depth = 0; card && depth < 7 && card !== document.body; depth += 1, card = card.parentElement) {
          ...
          const price = /(?:₩|￦)\\s*[\\d,]+|[\\d,]+\\s*원/.test(text);
    ...
    if (naverPortalSource && String(source.store || "") === "네이버 패션타운") {
      ...
      const approval = await september10VerifyApprovedNaverDomesticProducts(finalized?.products || [], {
        articleNumber,
        brand,
        title,
    ...
    if (Array.isArray(analyzed?.products)) {
      ...
      for (const product of inspectedProducts) {
        ...
        try {
          const productOpened = await september10ClickRenderedProductCard(searchWindow, product.url, resolvedSearchUrl);
          ...
          await september10OpenRenderedSizeOptions(searchWindow);
          const stockSelectors = september10RenderedStockSelectors(source.store);
...
async function september10AddRenderedSearchCounts(data, articleNumber, brand = "", title = "", generation = domesticSearchGeneration) {
  ...
  for (const source of data.sources) {
    ...
    // accuracy-first fallback order: code -> title -> title+code.
      ...
      // as though the product were absent.
        const queryResult = await september10RenderedSearchSourceResult(
...
function september10BrandsWithOfficialDomainStatus(brands, registry) {
...
async function september10EnsureOfficialDomainRegistry(brands) {
...
function september10SafeOfficialDomainRegistry(brands) {
...
function september10OfficialDomainAuditSnapshot(registry, extra = {}) {
...
function september10SendOfficialDomainAuditProgress(registry, extra = {}) {
...
async function september10ExportNaverOfficialStoreNotFoundExcel(registry) {
...
function september10CreateOfficialDomainAuditWindow() {
...
async function september10LoadAuditPage(auditWindow, url) {
  let timeout;
  try {
    await Promise.race([
...
async function september10CompareOfficialBrandLogos(sourceLogoUrl, candidateLogoUrls) {
...
async function runSeptember10DomesticSearch(_event, input) {
  const searchGeneration = domesticSearchGeneration;
    ...
    try {
      ...
      let matched = data;
      try {
        matched = await september10AddMatchConfidence(matched, input || {});
      } catch (error) {
        rememberWarning("match_confidence", error);
      }
      if (input?.verifyLinkCounts === true) {
        try {
          matched = await september10AddRenderedSearchCounts(
            matched,
            searchArticleNumber,
            searchBrand,
            searchTitle,
...
ipcMain.handle("seller:start-brand-export-monitor", () => {
  ...
  ipcMain.handle("domestic:search", (_event, input) => {
    // Popular, brand and category rows all use this IPC. Route them through
    // the proven September 10 direct search/detail/stock collector.
    if (typeof runSeptember10DomesticSearch === "function") {
      return runSeptember10DomesticSearch(_event, input);
    }
