...
test("상품카드는 사용자 화면을 빼앗지 않고 백그라운드에서 hover 후 클릭한다", () => {
  ...
  assert.doesNotMatch(clickFlow, /searchWindow\.show\(\)/);
  assert.doesNotMatch(clickFlow, /searchWindow\.focus\(\)/);
  assert.match(clickFlow, /await wait\(650\)/);
  assert.match(clickFlow, /sendInputEvent\(\{ type: "mouseMove"/);
  assert.match(clickFlow, /sendInputEvent\(\{ type: "mouseDown"/);
  assert.match(clickFlow, /sendInputEvent\(\{ type: "mouseUp"/);
});
...
test("선택 행 일괄 검색은 같은 브랜드와 품번 결과를 재사용한다", () => {
  ...
  assert.ok((rendererSource.match(/forceRefresh: false/g) || []).length >= 1);
});
