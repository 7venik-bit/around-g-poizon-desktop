...
test('official Open, exact manual links, and credential recovery survive every release transformation', async t => {
  ...
  const output = run(['--test', '--test-reporter=tap', 'tests/retailer-open-account-regression.test.mjs', 'tests/naver-save-login.test.mjs', 'tests/naver-password-route.test.mjs', 'tests/domestic-retailer-runtime.test.mjs', 'tests/store-save-recovery.test.mjs', 'tests/shopping-accounts.test.mjs', 'tests/shopping-accounts-ui.test.mjs', 'tests/naver-detail-stage-separation.test.mjs', 'tests/naver-batch-auth-restriction.test.mjs']);
