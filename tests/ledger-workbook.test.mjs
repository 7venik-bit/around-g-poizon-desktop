...
test('read-only bridge returns every sheet without calling any sheet mutator',async()=>{
 ...
 const sheet={getSheetId:()=>1,getName:()=> '1-구매완료',isSheetHidden:()=>false,getMaxRows:()=>1000,getMaxColumns:()=>30,getFrozenRows:()=>2,getFrozenColumns:()=>0,getDataRange:()=>({getDisplayValues:()=>[['001','#VALUE!']],getFormulas:()=>[['','=1+" "']],getBackgrounds:()=>[],getFontColors:()=>[],getFontWeights:()=>[],getNumberFormats:()=>[],getNotes:()=>[],getMergedRanges:()=>[]})};
 const context={LockService:{getScriptLock:()=>({waitLock(){},releaseLock(){}})},PropertiesService:{getScriptProperties:()=>({getProperty:()=> 'test'})},ContentService:{MimeType:{JSON:'json'},createTextOutput:s=>({setMimeType:()=>JSON.parse(s)})},DriveApp:{getFileById:()=>({getLastUpdated:()=>new Date('2026-09-21')})},SpreadsheetApp:{openById:()=>({getSheets:()=>[sheet],getName:()=> '원본'})},UrlFetchApp:{fetch:()=>({getResponseCode:()=>200,getContent:()=>[...bytes]})},ScriptApp:{getOAuthToken:()=> 'test'},Utilities:{DigestAlgorithm:{SHA_256:1},computeDigest:()=>[...Buffer.from(checksum,'hex')],base64Encode:()=>bytes.toString('base64')}};
