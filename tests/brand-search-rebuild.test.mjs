...
test("brand workflow connects directly and searches English before Korean fallback", () => {
  ...
  assert.doesNotMatch(workflow, /typeSellerBrandWithRealKeyboard\(candidate\.frame/);
  ...
  const exactFilter = workflow.indexOf("applyExactSellerBrandFilter(candidate.frame");
  const visibleAfterFilter = workflow.indexOf("sellerWindow.showInactive();", exactFilter);
  const physicalSort = workflow.indexOf("performPhysicalSellerSortAndExport(candidate.frame", exactFilter);
  assert.ok(exactFilter >= 0 && visibleAfterFilter > exactFilter && visibleAfterFilter < physicalSort);
  assert.ok(physicalSort > exactFilter);
  assert.match(main, /"PHYSICAL_SEARCH_BUTTON_CLICKED"/);
  assert.doesNotMatch(workflow, /runSellerSearch\(candidate\.frame/);
  ...
  assert.match(workflow, /const result = exactBrandFilter/);
