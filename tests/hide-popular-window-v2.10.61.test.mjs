...
test("popular-product collection minimizes Seller Center during normal capture", () => {
  assert.ok(captureBlock);
  assert.match(captureBlock, /openSellerCenterWindow\(SELLER_CENTER_URL, \{ visible: false \}\)/);
  assert.match(captureBlock, /minimizeSellerAutomationWindow\("POIZON 로그인 세션을 백그라운드에서 확인 중입니다\."\)/);
  assert.match(captureBlock, /minimizeSellerAutomationWindow\("POIZON 인기상품 200건을 백그라운드에서 수집 중입니다\."\)/);
  assert.doesNotMatch(captureBlock, /sellerWindow\.maximize\(\);\s*sellerWindow\.show\(\)/);
...
test("Seller Center appears only when login attention is required", () => {
  ...
  assert.match(captureBlock, /sellerWindow\.isMinimized\(\)\) sellerWindow\.restore\(\)/);
...
test("release metadata is 2.10.330", () => {
  assert.equal(JSON.parse(packageSource).version, "2.10.330");
  assert.equal(JSON.parse(lockSource).version, "2.10.330");
  assert.equal(JSON.parse(lockSource).packages[""].version, "2.10.330");
});
