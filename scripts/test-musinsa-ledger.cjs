...
app.whenReady().then(async()=>{
  ...
  const card=(size,line)=>`<article data-order-item-id="${line}"><a href="/products/10001"><img width="20" height="20" alt="테스트 운동화" src="https://www.musinsa.com/fake.svg"></a><a href="/products/10001">테스트 운동화</a><a href="/brands/test">테스트</a><p>옵션: ${size}</p><p>수량: 2개</p><p>실결제금액: 62,330원</p></article>`;
  isolated.protocol.handle('https',request=>{
    ...
    else if(url.pathname==='/products/10001'){productReads++;html='<h1>테스트 운동화</h1><p>현재 판매가 1원</p><p>품번: AB123-001</p>';}
    else if(url.pathname==='/fake.svg')return new Response('<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"/>',{headers:{'content-type':'image/svg+xml'}});
  ...
  const context=createContext({...page,...flow,...purchase,URL,setTimeout,clearTimeout,wait,BrowserWindow:FixtureWindow,
    ...
    fetch:async(url,options)=>{assert.equal(url,'https://script.google.com/macros/s/offline/exec');googleWrites++;const body=JSON.parse(options.body);assert.equal(body.row.purchasePrice,62330);return {json:async()=>({ok:true,rowNumber:45})};},AbortSignal,
  });
