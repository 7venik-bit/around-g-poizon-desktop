...
function replaceAny(candidates, after, label) {
  ...
  throw new Error(`Seller pagination patch target not found (${label}): ${candidates.join(" | ")}`);
}

replaceAny(
  ["const sellerPageDelayMs = 2_500;", "const sellerPageDelayMs = 7_000;"],
  "const sellerPageDelayMs = 12_000;",
  "page delay",
);
replaceAny(
  ["const sellerBatchPauseMs = 10_000;", "const sellerBatchPauseMs = 30_000;"],
...
replaceAny(
  ["서버 보호를 위해 10초 휴식 중", "서버 보호를 위해 30초 휴식 중"],
  "서버 보호를 위해 45초 휴식 중",
...
if (!source.includes("const sellerPageSettleMs = 10_000;")) {
  if (source.includes("const sellerPageSettleMs = 5_000;")) {
    source = source.replace("const sellerPageSettleMs = 5_000;", "const sellerPageSettleMs = 10_000;");
    changed = true;
  } else {
    replaceOnce(
      "  const sellerPageResponseAttempts = 360; // 360 × 250ms = 90 seconds\n",
      "  const sellerPageResponseAttempts = 360; // 360 × 250ms = 90 seconds\n  const sellerPageSettleMs = 10_000;\n",
      "post-navigation settle constant",
    );
  }
}
...
if (changed) {
  ...
  console.log("Applied Seller Center pacing: 12s/page, 45s/10 pages, 90s response wait, 10s settle, full-row readiness.");
} else {
