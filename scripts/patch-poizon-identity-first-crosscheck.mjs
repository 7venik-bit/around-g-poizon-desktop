...
identity = identity.replace(
  ...
  "  // SPU exact match is always first. If SPU text differs but the exact normalized article\n  // points to one and only one SPU, treat it as the same product for recognition.\n  // Multiple SPUs for the same article remain blocked as an identity conflict.\n  const articleSpus = new Set(candidates.map(productSpu).filter(Boolean));\n  if (articleSpus.size > 1) return fail('식별자 충돌');"
);
identity = identity.replace("  return decide(candidates, '상품번호');", "  return decide(candidates, productSpu(query) && articleSpus.size === 1 && !articleSpus.has(productSpu(query)) ? '상품번호(고유)' : '상품번호');");
await save('services/poizon-product-identity.mjs', identity);

let live = await read('services/live-poizon-crosscheck.mjs');
if (!live.includes("from './poizon-product-identity.mjs'")) {
...
console.log('POIZON identity-first crosscheck applied: recognize product first, compare sales second, and classify only unresolved identities as Excel-missing.');
