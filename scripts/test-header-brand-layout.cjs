...
function measure() {
  ...
  const ids = ['notification-open','update-check','export-button'];
  const buttons = ids.map((id) => ({ id, ...box(q('#'+id)) }));
...
(async () => {
  ...
  await writeFile(fixture, source.replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi,''), 'utf8');
  ...
  await win.loadFile(fixture); win.webContents.debugger.attach('1.3');
  ...
  const widths = [980,1040,1220,1426,1666,1920,2560];
  for (const width of widths) for (const scale of [1,1.25,1.5]) {
    ...
    for (const state of ['normal','updating']) {
      ...
      const text = state==='normal' ? '자동 업데이트' : '업데이트 다운로드 중…';
  ...
  for (const r of results) if (baseline && JSON.stringify(r.lamps)!==JSON.stringify(baseline.lamps)) r.errors.push('traffic light animation changed');
  ...
  if (!baseline || baseline.errors.length===0) throw new Error('Original screenshot defect was not reproduced');
  console.log('PASS: actual installed HTML/CSS, 42 viewport/scale/state cases, original defect reproduced.');
  win.destroy(); await rm(fixture,{force:true}); clearTimeout(deadline); app.exit(0);
})().catch(async (error)=>{
  console.error(error.stack||error);
  try { if(win&&!win.isDestroyed()){ await screenshot('failure'); win.destroy(); } }catch{}
  if(fixture) await rm(fixture,{force:true}).catch(()=>{});
